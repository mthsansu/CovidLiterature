...
write.csv(df, paste0(path_queries, '\\base_queries_articles_coronavirus_1.csv'))

# Certaines requetes sont à plus de 2000, on divise les queries en des plus fines
...
write.csv(df, paste0(path_queries, '\\base_queries_articles_coronavirus_2.csv'))

# On affine encore

jour_red <- "Plos One, International Journal Of Environmental Research And Public Health, Sustainability Switzerland"
list_journaux_red <- as.list(strsplit(jour_red, ", ")[[1]])
list_journaux_red <- as.character(unique(list_journaux_red))
req_cov <- '"COVID-19" OR "Coronavirus" OR "Corona virus" OR "2019-nCoV" OR "SARS-CoV" OR "MERS-CoV" OR "Severe Acute Respiratory Syndrome" OR "Middle East Respiratory Syndrome" AND '
req_y2 <- '( LIMIT-TO ( PUBYEAR,2021) )'
req_jour1 <- ' AND ( LIMIT-TO ( EXACTSRCTITLE,"'
req_jour2 <- '" )'
df <- data.frame (journal  = rep(list_journaux_red,2), req_journal = rep(list_journaux_red,2),
                  req_year = rep(c(req_y2),each = 6), req_tot = rep(req_cov,6))
df$req_journal <- paste0(req_jour1, df$req_journal,req_jour2)
df$req_tot <- paste0(df$req_tot, df$req_year,df$req_journal)
req_res <- c(') AND ( LIMIT-TO ( EXACTKEYWORD,"COVID-19" ) )',') AND ( LIMIT-TO ( EXACTKEYWORD,"Adult" ) )',
             ') AND ( LIMIT-TO ( EXACTKEYWORD,"COVID-19" ) )',') AND ( EXCLUDE ( EXACTKEYWORD,"COVID-19" ) )',
             ') AND ( EXCLUDE ( EXACTKEYWORD,"Adult" ) )',') AND ( EXCLUDE ( EXACTKEYWORD,"COVID-19" ) )')
for (i in 1:6) {
  df[i,"req_tot"] <- paste0(df[i,"req_tot"],req_res[i])
}
write.csv(df, paste0(path_queries, '\\base_queries_articles_coronavirus_3.csv'))
